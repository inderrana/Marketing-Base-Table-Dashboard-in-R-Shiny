...
The Weighted RFM metric encapsulates information from the three previous metrics which are weighted and represented in one coefficient, where 1.6 represents the first percentile of gamblers for the metric, i.e. those with the highest score or summation of the metric(`recency_score`,`frequency_score`, and `monetary_value_score`). The three metrics are then transformed into a numeric value with the same ranking system as weighted RFM.`weighted_rfm` aggregates these three intermediary metrics to give a comprehensive measure of each gambler RFM value.  
...
The loyality metric categorizes each gambler in the dataset in one of the following bins: Regular Customers, Hesitant Customers, Needs Attention, Potential Loyalists, and Loyalists. The tier in which a gambler falls within is dictated by their weighted RFM scored classified as follows: Weighted RFM score greater than 1.6: Loyalists, weighted RFM between 1.6 and 1.3: Potential Loyalists, weighted RFM between 1.3 and 0.7: Regular Customers, weighted RFM between 0.7 and 0.4: Hesistant customers, and finally weighted RFM below 0.4: Needs Attention.   
