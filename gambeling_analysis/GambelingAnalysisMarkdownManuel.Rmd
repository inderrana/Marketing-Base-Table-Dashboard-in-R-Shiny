...
```{r,echo=FALSE,cache=FALSE, warning=FALSE, comment=FALSE, warning=FALSE,message=FALSE}
profit_to_rfm <- ggplot(base_table,aes(x=rfm_value,y=profit,size=amount)) + 
  geom_point() + 
  theme_bw() + 
...
It is visible that there's actually a somewhat weak correlation between profit for the brand and RFM value of the gambler. Additionally, the gamblers with a mid-range RFM value account are the most profitable gamblers for Bwin. The pearson correlation coefficient can be found below.

```{r, echo=FALSE, include=TRUE,results=TRUE,comment=FALSE, warning=FALSE,message=FALSE}
cor(base_table$profit,base_table$rfm_value)
```
...
The most relevant insights from a business perspective that our analysis brought to the forefront are as follows:

* We have a small population of loyalists with a high betting frequency, who seldomly win big making them our most profitable gamblers.  
